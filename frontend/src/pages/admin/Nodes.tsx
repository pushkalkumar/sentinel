...
export default function Nodes() {
  ...
  return (
    <>
      <PageHeader title={`${online} of ${nodes.length} nodes online`} />
      <div className="flex flex-col gap-4">
          ...
          <Panel title="Provisioning">
            {picked ? (
              <dl className="grid grid-cols-[8rem_1fr] gap-y-2 text-sm">
                <dt className="text-ink-3 self-center">SSID</dt><dd className="font-mono text-xs text-ink">SENTINEL-{picked.id}</dd>
                <dt className="text-ink-3 self-center">Label</dt><dd className="text-ink">{picked.label}</dd>
                <dt className="text-ink-3 self-center">Position</dt><dd className="font-mono text-xs text-ink-2">{picked.lat.toFixed(5)}, {picked.lng.toFixed(5)}</dd>
                <dt className="text-ink-3 self-center">Role</dt><dd className="text-ink-2">{picked.is_gateway ? 'Gateway: uplinks the mesh to the backend' : 'Mesh node: relays to the gateway over UDP'}</dd>
                <dt className="text-ink-3 self-center">Neighbours</dt><dd className="font-mono text-xs text-ink-2">{picked.neighbours.join(', ') || 'none'}</dd>
                <dt className="text-ink-3 self-center">Firmware</dt><dd className="font-mono text-xs text-ink-2">{picked.fw_version}</dd>
              </dl>
            ) : (
        ...
        <Panel title="Mesh log" live padded={false}>
          <MeshLog lines={14} className="max-h-80" />
        </Panel>
      </div>
    </>
  )
