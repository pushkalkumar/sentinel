...
export interface NodeDotProps {
  ...
  flash?: boolean
  compact?: boolean
  /** `hover` keeps the map quiet: the label shows on hover, focus or selection only. */
  labels?: LabelMode
  onSelect?: (id: string) => void
}

/** DESIGN §6.5, muted per DESIGN_V2: 9px dot, square for gateways, one calm ring for alerting, two for LOCAL_FIRE. */
export function NodeDot({ node: n, mode, selected, highlighted = false, flash = false, compact = false, labels = 'always', onSelect }: NodeDotProps) {
  ...
  const shape = {
    ...
    stroke: offline ? OFFLINE_STROKE : 'rgba(11,10,9,0.6)',
    strokeWidth: offline ? 1.25 : 1.5,
  }
  const labelFill = fire ? ALARM : offline ? 'var(--color-ink-4)' : 'var(--color-ink-2)'
  const quiet = labels === 'hover' && !selected
  ...
  return (
    ...
    >
      ...
      {fire && (
        <>
          <circle r={11} fill="none" stroke={ALARM} strokeWidth={1} opacity={0.5} className="pulse-ring-alarm" style={PULSE_STYLE} />
          <circle r={16} fill="none" stroke={ALARM} strokeWidth={1} opacity={0.3} className="pulse-ring-alarm" style={{ ...PULSE_STYLE, animationDelay: '0.6s' }} />
          <circle r={11} fill="none" stroke={ALARM} strokeWidth={1} opacity={0.25} />
        </>
      )}
      {alerting && !offline && (
        <circle r={10} fill="none" stroke={bandColor} strokeWidth={1} opacity={0.3} className="pulse-ring" style={PULSE_STYLE} />
      )}
      {highlighted && <circle r={12} fill="none" stroke="var(--color-signal)" strokeWidth={1} opacity={0.5} className="pulse-ring" style={PULSE_STYLE} />}
      {flash && <circle r={10} fill="none" stroke="var(--color-signal)" strokeWidth={1.5} opacity={0.5} />}
      {selected && <circle r={9} fill="none" stroke="var(--color-accent)" strokeWidth={1} opacity={0.9} />}
      {n.is_gateway
        ? <rect x={-DOT_R} y={-DOT_R} width={DOT_R * 2} height={DOT_R * 2} rx={1.5} {...shape} />
      ...
      {mode === 'air' && !compact && n.latest && !offline && (
        <text x={DOT_R + 8} y={19} fontFamily="var(--font-mono)" fontSize={11} fill={fill} className={labelClass}>
